...
- (void)openTZImagePicker:(NSDictionary *)options callback:(RCTResponseSenderBlock)callback {
    ...
    NSInteger videoMaxSize = [options sy_integerForKey:@"videoMaxSize"];
    ...
    [imagePickerVc setDidFinishPickingVideoHandle:^(UIImage *coverImage, PHAsset *asset) {
        ...
        [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
            if ([asset isKindOfClass:[AVURLAsset class]]) {
                ...
                NSNumber *size;
                [urlAsset.URL getResourceValue:&size forKey:NSURLFileSizeKey error:nil];//size is 43.703005
                if ((int)[size floatValue]/(1024.0*1024.0) > videoMaxSize) {
                    ...
                    [[TZImageManager manager] getVideoOutputPathWithAsset:_asset presetName:AVAssetExportPresetMediumQuality success:^(NSString *outputPath) {
                        ...
                        if (weakSelf.canCallback) {
                            weakSelf.canCallback = NO;
                            callback(@[[NSNull null], @[[self handleVideoData:outputPath asset:_asset coverImage:coverImage quality:quality]]]);
                            [weakPicker dismissViewControllerAnimated:YES completion:nil];
                            [weakPicker hideProgressHUD];
                            weakSelf.canCallback = NO;
                        }
